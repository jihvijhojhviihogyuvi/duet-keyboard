...
import { PivotComposer } from '@/components/lilt/pivot-composer'
import { useComposer } from '@/hooks/use-composer'
import { NAME_TESTS } from '@/lib/pivot-engine'
    ...
    <Tabs value={tab} onValueChange={(value) => setTab(String(value))} className="min-h-svh gap-0">
      <header className="lab-header">
        <div className="lab-header-inner">
          ...
          <div className="lab-header-actions">
            <Button variant="quiet" size="touch" onClick={() => setHelpOpen(true)} aria-label="How Pivot works"><CircleHelp data-icon="inline-start" /><span className="help-label">How it works</span></Button>
      ...
      <main className="lab-main">
        <section className="lab-intro" aria-labelledby="lab-heading">
          <div className="intro-title-group"><p className="experiment-label font-mono"><span aria-hidden="true" />EXPERIMENT 02 — PIVOT</p><h1 id="lab-heading" className="lab-heading text-balance">Not predicted. <span>Written.</span></h1></div>
          <p className="lab-intro-copy text-pretty">A gesture alphabet for names, nonsense,<br className="desktop-break" /> and everything only you would say.</p>
        </section>

        <TabsContent value="write"><PivotComposer composer={draft} compact={compact} onExpand={() => setCompact(false)} /></TabsContent>
        <TabsContent value="test"><PivotComposer key={`${challengeIndex}-${practiceKey}`} composer={practice} compact={compact} onExpand={() => setCompact(false)} challengeIndex={challengeIndex} onRestart={restartPractice} onNext={nextPractice} /></TabsContent>
